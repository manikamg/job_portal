 ...
 $npost=query("SELECT `id`, `admin`, `stfid`, `pic`, `name`, `contact`, `emailid`, `address`, `farmname`, `farmno`, `pass`, `status`, `ip`, `datetime` FROM `client` WHERE `status`='active'");
...
$number_post=mysqli_real_escape_string($db,$_POST['number_post']);
 $loc=(explode(",",$location));
   foreach ($loc as $key => $value) {
 	...
 	$sql=query("INSERT INTO `post_job`(`postid`,`jobtyp`,`adminid`,`compid`, `postname`,`post_number`,`priority`, `exp`, `location`, `gender`, `highestqualification`, `joiningprocess`, `ta`,`postdetails`, `ssalary`, `esalary`, `sdate`, `edate`, `alernumber`,`adm_approve`, `stf_approve`,`ip`) VALUES ('$pcode','$jobtyp','$id','$company','".strtolower($postname)."','$number_post','".strtolower($priority)."','".strtolower($exp)."','".strtolower($location)."','".strtolower($gender)."','".strtolower($highestqualification)."','".strtolower($joiningprocess)."','".strtolower($ta)."','".strtolower($postdetails)."','".strtolower($startsalary)."','".strtolower($endsalary)."','".($openingdate)."','".($enddate)."','".($alternative_number)."','approved','approved','".$_SERVER['REMOTE_ADDR']."')")or die(mysqli_error($db));
 	if($sql>0){
...
<div class="box box-danger">
 ...
 <label>Job Code</label>: <input type="text" name="pcode" value="<?php echo mt_rand(); ?>" maxlength="10" required="required" >
              <div class="form-group">
               <label>Job Type <b style="color: red;">*</b></label>
                <div class="input-group">
                  <div class="input-group-addon">
                    <i class="fa fa-circle-o text-red"></i>
                  </div>
                  <select class="form-control" name="jtyp">
                  <?php $jobtyp=query("SELECT `id`, `name` FROM `jobtype` ORDER BY `timedate` ASC"); while($abc=mysqli_fetch_array($jobtyp)){?>
                  <option value="<?php echo $abc['id']; ?>"><?php echo ucwords($abc['name']); ?></option><?php }?>
          </select>
          <span class="input-group-btn">
                      <button type="button" class="btn btn-info btn-flat" onclick="window.open('job-type','_self');">Add New</button>
                    </span>
                </div>
              </div> 
               <div class="form-group">
