<?php 
 ...
 if(isset($_POST['submit'])){ 
  ...
  if($sql>0){echo "<script>window.open('add-consultancy?id=".$adminidx."&rid=".$lid."&success','_self')</script>";}else{echo "<script>alert('Server Is Busy')</script>";}
}
?>
  <div class="container">
    <section class="content-header">
     <h1><small><a href="dashboard?id=<?php echo trim($_GET['id']); ?>"><< Back</a>< Add New Consultacy</small></h1>
      <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-md-2 col-xs-12"></div>
         <div class="col-md-8 col-xs-12">
            ...
            <p> Registration success</p><hr/>
            <?php $cs=query("SELECT `loginid`, `password` FROM `admins` WHERE `id`='".trim($_GET['rid'])."'");
            $runc=mysqli_fetch_array($cs);?>
            <ul><li>LoginId<br/><?php echo $runc['loginid'] ?></li>
                <li>Password<br/><?php echo $runc['password'] ?></li></ul>
            </div>
            <?php }?>
            ...
            <div class="box-body">
              <form role="form" enctype="multipart/form-data" method="post">
                 ...
                 <div class="form-group"> 
                  ...
                  <input type="text" class="form-control" name="compname" placeholder="Enter Here" required="required"></div> 
                 <div class="form-group"><label>Owner Name <b style=color:red;></b></label> 
                  <input type="text" class="form-control" name="oname" placeholder="Enter Here" required="required"></div>
                  <div class="form-group"><label>Contact <b style=color:red;></b></label> 
                  <input type="text" class="form-control" name="mob" placeholder="Enter Here" maxlength="10" minlength="10" pattern="[0-9]+" required="required"></div>
                  <div class="form-group"><label>Email Id <b style=color:red;></b></label> 
                  <input type="email" class="form-control" name="mail" placeholder="Enter Here"></div>
                  <div class="form-group"><label>Address </label> 
                  <input type="text" class="form-control" name="address" placeholder="Enter Here" ></div>
                  <div class="form-group"><label>About Us </label> 
                  <textarea class="form-control" placeholder="enter here" name="about"></textarea></div>
                <div class="form-group">
