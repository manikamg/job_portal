 ...
 include_once 'head.php';?>
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="row">
        <?php $apply=sqlquery("SELECT * FROM `telli_calling` WHERE `show`=2 AND `rand`='".trim($_GET['id'])."' GROUP BY `rand`");
        if(mysqli_num_rows($apply)>0){
          while($ab=mysqli_fetch_array($apply)){ ?>
        <a href="telling-calling-list-lead-full-report?<?php echo md5(uniqid()); ?>&<?php echo md5(uniqid()); ?>&id=<?php echo $ab['rand']; ?>&<?php echo md5(uniqid()); ?>"><div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box"><span class="info-box-icon bg-aqua"><i class="fa fa-map-signs"></i></span>
      ...
      <?php }
        ...
        echo '<div align=center><b>No Lead Found..!</b></div>';
      } ?>
      </div>
    </section>
  <?php }else{?>
...
<div class="box-header with-border"><h3 class="box-title">Telling Calling Data Manage</h3></div> <div class="box-body"><div class="box-group" id="accordion">
<table id="example1" class="table table-bordered table-striped"><thead><tr><th>Status</th><th>Name</th><th>Location</th><th>H Edu.</th><th>Exp<br/>Details</th><th>View</th></tr></thead><tbody>
<?php $apply=sqlquery("SELECT `id`, `adm`, `member`, `name`, `number`, `location`, `hedu`, `exp_type`, `exp`, `details`, `post_looking`, `comment`, `timedate`, `reply_date`, `wronno`, `norespons`, `show`, `finalstatus` FROM `telli_calling` WHERE `member`='$id' AND `show`=2"); while($ab=mysqli_fetch_array($apply)){ ?>
  <tr>
