...
if (isset($_POST['submitemp'])) {
	...
	$loginid = 'jobportal' . rand('1111111', '9999999');
	...
	if (empty(mysqli_num_rows($check))) {
		...
		$chk = mysqli_query($db, "SELECT `id`, `admid`, `name`, `email`, `loginid`, `password`, `pic`, `is_super`, `created_at`, `updated_at` FROM `admins` WHERE `admid`='$eid'");
		...
		if (empty(mysqli_num_rows($chk))) {
			...
			$sql = mysqli_query($db, "INSERT INTO `admins`(`admid`,`name`, `email`, `loginid`, `password`,`is_super`) VALUES ('$eid','$ename','$email','$loginid','$pass',1)") or die(mysqli_error($db));
			...
			$sql=mysqli_query($db,"INSERT INTO `admins`(`admid`,`name`, `email`, `loginid`, `password`,`is_super`) VALUES ('$eid','".$abc['name']."','".$abc['email']."','".$abc['loginid']."','".$abc['password']."',1)")or die(mysqli_error($db));
		}
