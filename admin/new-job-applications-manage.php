  ...
  <div class="container" style="padding-right: 50px;">
    <section class="content-header">
      <h1><small><a href="dashboard?id=<?php echo trim($_GET['id']); ?>">
            << Back</a> New Job Applications</small></h1>
      <ol class="breadcrumb">
              ...
              <tbody>
                ...
                while ($run = mysqli_fetch_array($joblist)) { ?>
                  <tr>
                    ...
                    <td><small>
                        <?php echo (postName($run['jobid'])) . '&nbsp; <br/> &nbsp;' . (postCode($run['jobid'])); ?>
                      </small></td>
                    <td><small>
                        <?php echo (postNametoLocation($run['jobid'])); ?>
                      </small></td>
                    <td><b><small>
                          <?php if ((companyname($run['compid']) != 0)) {
                            echo ucwords(companyname($run['compid']));
                          } else {
                            echo 'Not Available';
                          } ?>
                        </small></b></td>
                    <td><small>
                        <?php if ((companynameRgno($run['compid']) != 0)) {
                          echo ucwords(companynameRgno($run['compid']));
                        } else {
                          echo 'Not Available';
                        } ?>
                      </small></td>
                    <td><small>
                        <?php echo date('d-m-Y', strtotime(postApplylastdate($run['jobid']))); ?>
                      </small></td>
                    <td><span class="label label-warning">
                        <?php echo ucwords(postHighQual($run['jobid'])); ?>
                      </span></td>
                    <td><span class="label label-danger">
                        <?php echo ucwords(postHighQual($run['jobid'])); ?>
                      </span></td>
                    <td><button type="button"
                        onclick="window.open('new-application-details?id=<?php echo trim($_GET['id']); ?>&jid=<?php echo base64_encode($run['jobid']) ?>','_self')"
