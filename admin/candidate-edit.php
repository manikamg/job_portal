  ...
  $address = mysqli_real_escape_string($db,$_POST['address']);
      ...
      $pass = rand(111111,99999);
       //imagupload
           ...
           $img=$_FILES['pic']['name'];
$newpath ="../assets/candidate/".$_FILES['pic']['name'];
move_uploaded_file($oldpath, $newpath);
      $sql=mysqli_query($db,"UPDATE `candidate` SET `pic`='$img',`name`='$fullname',`contact`='$contact',`fname`='$fname',`mname`='$mname',`emailid`='$email',`address`='$address',`pass`='$pass' WHERE id='".trim($_POST['did'])."'")or die(mysqli_error($db));
  if($sql>0){
    echo "<script>window.open('candidate-edit?id=".trim($_GET['id'])."&success','_self')</script>";
  }else{
      echo "<script>alert('Server Busy')</script>";
  
  }
  }
  $sqll=mysqli_query($db,"SELECT  * FROM `candidate` WHERE `id`='".trim($_GET['id'])."'");
            ...
            <div class="box-body">
              <form role="form" enctype="multipart/form-data" method="post" action="#">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" rows="3" name="fname" value="<?php echo $run['fname'] ?>" placeholder="Enter Here"></textarea>
                </div> 
                <div class="form-group">
                 ...
                 <input type="text" class="form-control" name="mname" value="<?php echo $run['mname'] ?>" placeholder="Enter Here" required="required">
                   ...
                   <div class="form-group">
                  <label>Address</label>
                  <textarea class="form-control" rows="3" name="address" value="<?php echo $run['address']; ?>" placeholder="Enter Here"></textarea>
                </div>
                   <div class="form-group">
                  <label>Attach Pic</label><br/>
                  <?php if(!empty($run['pic'])){ ?>
                  <img src="../assets/candidate/<?php echo $run['pic'] ?>" style="width: 60px;height: 60px;" >
                  <?php }?>
                  <br/>
