 ...
 <h1><small></small>
  ...
  <table id="" class="table table-fixedheader table-bordered table-striped">
   ...
   <tr>
    ...
    <td><?php echo companyname($row['compid']); ?></td>
    <td><b><?php echo companyOwner($row['compid']); ?></b></td>
    <td><b><?php echo strtoupper(companynameRgno($row['compid'])); ?></b></td>
    <td><?php echo date('d/M/Y',strtotime($row['timedate'])); ?></td>
    <td>

      <?php if($row['status']=='yes'){ echo '<span class="pull-right badge bg-green">Active</span>'; }elseif($row['status']=='no'){
