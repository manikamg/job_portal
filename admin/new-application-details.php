...
<div class="container" style="padding-right:50px;">
  <section class="content-header">
    <h1><small><a href="new-job-applications-manage?id=<?php echo trim($_GET['id']); ?>">
          << Back</a>
            < Selected Applications</small>
    </h1>
    <ol class="breadcrumb">
          ...
          <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
              <button type="button" class="btn btn-box-tool" data-widget="remove"
                onclick="window.open('new-application-details?jid=<?php echo $_GET['jid']; ?>','_self')"><i
      ...
      <div class="box">
        <div class="box-header">
          <table class="table table-bordered" style="background-color: gray;color: #fff;">
            ...
            <tr>
              <td>
                <?php echo (companyname($dsql['compid'])) . '<br/>' . (companynameRgno($dsql['compid'])); ?>
              </td>
              <td>
                <?php echo ucwords(postPriority($dsql['compid'])); ?>
              </td>
              <td>
                <?php echo ucwords(postHighQual($dsql['jobid'])); ?>
              </td>
              <td>
                <?php echo ucwords(postName($dsql['jobid'])); ?>
              </td>
              <td>
                <?php echo (postCode($dsql['jobid'])); ?>
              </td>
            </tr>
          </table>
        </div>
            ...
            <tbody>
              ...
              while ($run = mysqli_fetch_array($joblist)) {
                ...
                <tr>
                  ...
                  <td><small>
                      <?php echo (postCode($run['jobid'])); ?>
                    </small></td>
                  <td><small>
                      <?php echo date('d-m-Y', strtotime(($run['timedate']))); ?>
                    </small></td>
                  <td>
                      <!-- <?php if (empty($row['pic'])) { ?>
                        ...
                        <?php } else { ?>
                          <img
                      src="../assets/candidate/<?php echo ucwords(applicantPic($run['applicantid'])); ?>"
                      style="height: 60px;width: 60px;" />
                        <?php } ?> -->

                        <?php echo $run['applicantid']; ?>
                    </td>
                  <td><span class="label label-primary">
                      <?php echo ucwords(applicantName($run['applicantid'])); ?>
                    </span></td>
                  <td><small>
                      <?php echo ucwords(applicantHighqual($run['applicantid'], 'highestqualification')); ?>
                    </small></td>
                  <td><small>
                      <?php echo ucwords(applicantContact($run['applicantid'])); ?>
                    </small></td>
                  <td><a
                      href="https://consultancy.rmjob.in/assets/candidate/<?php echo (applicantResume($run['applicantid'])); ?>"
                      target="_blank"><img src="../assets/general/download.png" style="height:30px;width:30px;" /></a>
                  </td>
                </tr>
                <?php $i++;
              } ?>
